...
class TriggerTestCase(SignalNotifierTestBase):
	...
	def test_register_trigger_by_custom_signal(self):
		...
		# Create a backend
		simple1Backend = Backend.objects.filter(name=Messengers_name[0][0])[0]

		# Add that backend to trigger's subscription
		trigger.subscription.backends.add(simple1Backend)

		# Start test:
		# Sending signal, you should see the simple1Backend related output
		custom_signal.send(sender=TestModel, actor=self.user1) # Must print simple1Backend output
